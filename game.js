onload = function () {
    ...
    const text = 'Jumps array as input is shown. Each index stores the maximum islands we can jump ahead from current island. ' +
        'Find least number of moves needed to reach the last island and return jump to be taken on each island.<br>';
    ...
    function updateState() {
        ...
        //if body is enable to move
        if (player.body.enable) {
            if (onGround) {
                onGround = false;
                if(pos<moves.length) {
                    //velocity based on the value of jump required
...
//starting platform is at index 0
